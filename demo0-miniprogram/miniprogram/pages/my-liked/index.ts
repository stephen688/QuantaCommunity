...
import { getMyLikedList } from '../../services/user-content.service';
...
Page({
  ...
  async loadFirstPage(fromRefresh: boolean = false) {
    ...
    try {
      const res = await getMyLikedList(1, this.data.pageSize);
      if (!res.ok) {
  ...
  async loadMore() {
    ...
    try {
      ...
      const res = await getMyLikedList(next, this.data.pageSize);
      if (!res.ok) {
