...
async def test_record_and_fetch_roundtrip(tmp_path) -> None:
    ...
    await audit.record(
        DecisionLogEntry(comment_id=8, decision="rejected_moderation", mode=None, reason="命中敏感词")
    )
...
class SQLiteAudit:
    ...
    async def fetch_entries(self) -> list[_Row]:
        ...
        return [_Row.model_validate(dict(zip(("comment_id", "decision", "mode", "reason", "created_at"), r))) for r in rows]
```
...
async def _audit(deps: PipelineDeps, comment_id: int, d: Decision, reason: str, mode: str | None = None) -> None:
