...
- **命名**：模块/函数 snake\_case，类 PascalCase，常量 UPPER\_SNAKE；不搞单字母变量（循环索引除外）。命名既要见明知意，又不能过长。
...
> 核心闭环：**计划阶段（读总计划 → grill-me 对齐 → writing-plans 写计划）→ 动手前规划与阅读 → 写码中最小改动 → 完成后验证与交付**（对齐主流 AGENTS.md 实践）。任何任务都必须走完这个闭环；以下为硬性动作序列。

### 6.0 计划阶段（Planning；新任务/大功能必经）

> 触发条件：任何新任务、大功能、跨模块改动开始前。小修小改（typo/单点 bug/纯文档措辞）可跳过本节直接进 6.1。

1. **读总计划**：先读 `总计划.md`（项目根，**待创建**）——项目级路线图（阶段划分/任务依赖/里程碑）。确认本次任务在总计划中的位置与前后依赖，明确"这一刀切在哪"。
...
- **提交频率**：每完成一个可独立验证的小块就提交一次，不攒大 commit，微小改动无需提交。
