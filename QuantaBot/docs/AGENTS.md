...
- **命名**：模块/函数 snake_case，类 PascalCase，常量 UPPER_SNAKE；不搞单字母变量（循环索引除外）。
...
- 幂等去重以 **comment_id** 为键（不是 message id；message id 仅投递层辅助）。
- 同帖消息按 post_id 串行处理，组装上下文带"触发前最新楼层快照"防穿越。
...
**Don't**
- 不绕过主服务直连数据库写回复。
- 不在人格/策略变更后沿用旧口径记忆而不做处理（Qdrant payload 用 persona_version 标记失效）。
- 不实现"全链路一个超时"。
...
| v0.5 | 2026-09-13 | §1 补充 GitHub 仓库地址（https://github.com/stephen688/QuantaCommunity，QuantaBot 为其中子目录）与宿主主服务位置 |
