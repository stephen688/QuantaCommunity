...
import { Clock, Refresh, CircleCheck, CircleClose, ChatLineRound } from '@element-plus/icons-vue'
import { commentApi } from '../../api/comment'
import { buildCommentSnapshotFromReport, publicReadApi } from '../../api/publicRead'
...
const handleResultSource = [
  { value: 1, label: '删除评论', requiredAuthority: 'CONTENT_DELETE' },
  { value: 2, label: '警告用户' },
  { value: 3, label: '删除评论 + 警告用户', requiredAuthority: 'CONTENT_DELETE' },
  { value: 4, label: '驳回举报' },
]
...
async function fetchList() {
  ...
  try {
    const data = await commentApi.reportPage(buildPageParams())
...
async function openDetail(row) {
  ...
  commentDetail.value = buildCommentSnapshotFromReport(row)
  drawerVisible.value = true
  detailLoading.value = true
  try {
    const enriched = await publicReadApi.resolveCommentDetail(row)
    if (enriched) commentDetail.value = enriched
  } catch {
    commentDetail.value = buildCommentSnapshotFromReport(row)
  } finally {
...
async function openHandle(row) {
  ...
  handleTargetDetail.value = buildCommentSnapshotFromReport(row)
  handleVisible.value = true
  handleTargetLoading.value = true
  try {
    const enriched = await publicReadApi.resolveCommentDetail(row)
    if (enriched) handleTargetDetail.value = enriched
  } catch {
    handleTargetDetail.value = buildCommentSnapshotFromReport(row)
  } finally {
...
<template>
  <div class="report-section">
    ...
    <el-card shadow="never" class="table-card portal-panel">
      ...
      <div v-if="showListSkeleton" class="list-skeleton" aria-busy="true" aria-label="加载评论举报列表">
      ...
      >
        ...
        <el-table-column label="所属帖子" min-width="148" show-overflow-tooltip>
          <template #default="{ row }">{{ row.targetPostTitle || '—' }}</template>
        </el-table-column>
        <el-table-column label="评论摘要" min-width="140" show-overflow-tooltip>
          <template #default="{ row }">{{ row.targetCommentPreview || '—' }}</template>
        </el-table-column>
        <el-table-column label="举报说明" min-width="148" show-overflow-tooltip>
    ...
    >
      ...
      <div v-else-if="detailReport" class="portal-detail-stack">
        <div class="portal-detail-summary">
          ...
          <div class="portal-detail-summary-main">
            <h3 class="portal-detail-summary-title portal-detail-summary-title--clamp">
              {{ commentDetail?.postTitle || detailReport.targetPostTitle || '—' }}
            </h3>
            <div class="portal-detail-summary-meta">
              <span class="portal-badge portal-badge--muted">举报 {{ detailReport.id }}</span>
              <span class="portal-badge portal-badge--muted">评论 {{ detailReport.commentId }}</span>
        ...
        <template v-if="commentDetail">
          <el-descriptions class="portal-details-in-drawer" title="被举报评论（当前快照）" :column="1" border>
            <el-descriptions-item label="评论正文">
              <pre class="portal-readonly-block">{{ commentDetail.content ?? '—' }}</pre>
            </el-descriptions-item>
            <el-descriptions-item label="评论人 ID">{{ commentDetail.userId ?? '—' }}</el-descriptions-item>
            <el-descriptions-item label="帖子 ID">{{ commentDetail.contentId ?? '—' }}</el-descriptions-item>
            <el-descriptions-item label="关联回答 ID">
              {{ commentDetail.answerId ?? '—' }}
            </el-descriptions-item>
            <el-descriptions-item label="发布时间">{{ commentDetail.createTime ?? '—' }}</el-descriptions-item>
          </el-descriptions>
          <el-descriptions class="portal-details-in-drawer" title="所属帖子上下文" :column="1" border>
            <el-descriptions-item label="标题">{{ commentDetail.postTitle ?? '—' }}</el-descriptions-item>
            <el-descriptions-item label="帖子正文">
              <pre class="portal-readonly-block">{{ commentDetail.postContent ?? '—' }}</pre>
            </el-descriptions-item>
            <el-descriptions-item label="发布者 ID">{{ commentDetail.postPublishUserId ?? '—' }}</el-descriptions-item>
          </el-descriptions>
          <el-descriptions
            v-if="commentDetail.relatedAnswer"
            class="portal-details-in-drawer"
            title="被回复的回答（节选）"
            :column="1"
            border
          >
            <el-descriptions-item label="回答 ID">{{ commentDetail.relatedAnswer.answerId }}</el-descriptions-item>
            <el-descriptions-item label="作者 ID">{{ commentDetail.relatedAnswer.userId }}</el-descriptions-item>
            <el-descriptions-item label="正文">
              <pre class="portal-readonly-block">{{ commentDetail.relatedAnswer.content }}</pre>
            </el-descriptions-item>
          </el-descriptions>
        </template>
        <el-alert
          ...
          type="warning"
          title="未能加载评论与上下文"
          description="可能被删除或无权限查看，仍可依据工单字段处置。"
          :closable="false"
          show-icon
        />
      </div>
    ...
    >
      <template v-if="currentReport">
        <el-descriptions class="portal-details-in-drawer portal-dialog-detail-desc" title="举报工单" :column="1" border>
          <el-descriptions-item label="评论 ID">{{ currentReport.commentId }}</el-descriptions-item>
        ...
        <div v-loading="handleTargetLoading" class="report-handle-target">
          <template v-if="handleTargetDetail">
            <el-descriptions
            ...
            >
              ...
              <el-descriptions-item label="评论人 ID">{{ handleTargetDetail.userId ?? '—' }}</el-descriptions-item>
            </el-descriptions>
            <el-descriptions
              class="portal-details-in-drawer portal-dialog-detail-desc report-handle-post"
              title="帖子上下文"
              :column="1"
              border
            >
              <el-descriptions-item label="标题">{{ handleTargetDetail.postTitle ?? '—' }}</el-descriptions-item>
              <el-descriptions-item label="帖子正文">
                <pre class="portal-readonly-block">{{ handleTargetDetail.postContent ?? '—' }}</pre>
              </el-descriptions-item>
            </el-descriptions>
          </template>
          <el-alert
